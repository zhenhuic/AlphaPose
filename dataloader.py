...
class CustomDataWriter:
    def __init__(self, save_video=False,
        ...
        if opt.save_img:
            if not os.path.exists(opt.outputpath + '/vis'):
                os.mkdir(opt.outputpath + '/vis')
        if opt.save_crop_img:
            if not os.path.exists(opt.outputpath + '/crop'):
                os.mkdir(opt.outputpath + '/crop')
        ...
        while True:
            ...
            if not self.Q.empty():
                ...
                if boxes is None:
                    ...
                    if opt.save_crop_img:
                        crop_imgs = crop_local_img(orig_img, result)
                        for i, crop in enumerate(crop_imgs):
                            cv2.imwrite(os.path.join(opt.outputpath, 'crop', str(i) + im_name), crop)
